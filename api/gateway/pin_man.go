...
func (s *Server) pinContent(w http.ResponseWriter, r *http.Request) {
    ...
    if err := s.replicator.PinContent(hash); err != nil { s.writeError(w, http.StatusInternalServerError, "Failed to pin content", err); return }
    response := &pb.PinResponse{ Hash: hash.String(), Message: "Content pinned successfully" }
...
func (s *Server) unpinContent(w http.ResponseWriter, r *http.Request) {
    ...
    hash, err := hasher.HashFromString(hashStr)
    if err != nil { s.writeError(w, http.StatusBadRequest, "Invalid hash", err); return }
    if err := s.replicator.UnpinContent(hash); err != nil { s.writeError(w, http.StatusInternalServerError, "Failed to unpin content", err); return }
    response := &pb.PinResponse{ Hash: hash.String(), Message: "Content unpinned successfully" }
    s.writeJSON(w, http.StatusOK, response)
}
