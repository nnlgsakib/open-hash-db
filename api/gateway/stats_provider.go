...
func (s *Server) getNetworkStats(w http.ResponseWriter, r *http.Request) {
    if s.node == nil { s.writeError(w, http.StatusServiceUnavailable, "Network node not available", nil); return }
...
func (s *Server) getStats(w http.ResponseWriter, r *http.Request) {
    ...
    if err != nil { s.writeError(w, http.StatusInternalServerError, "Failed to get storage stats", err); return }
    storageStats := &pb.StorageStats{ContentCount: int64(len(content))}
    replicationStats := s.replicator.GetStats()
    stats := &pb.StatsResponse{ Storage: storageStats, Replication: replicationStats, Timestamp: timestamppb.Now() }
    s.writeJSON(w, http.StatusOK, stats)
}
