...
func (s *Server) streamChunksOptimized(w http.ResponseWriter, r *http.Request, metadata *pb.ContentMetadata) {
    ctx := r.Context()
    if metadata.IsErasureCoded { s.streamErasureCodedContent(w, r, metadata); return }
    ...
    if rangeHeader != "" { s.handleRangeRequestOptimized(ctx, w, r, metadata, rangeHeader); return }
    ...
    for _, chunkInfo := range metadata.Chunks {
        ...
        if err != nil { log.Printf("Aborting stream for %s invalid chunk hash: %v", string(metadata.Hash), err); return }
        if err := s.streamChunkWithBuffer(ctx, w, chunkHash, 0, int(chunkInfo.Size), buffer); err != nil {
            if !isClientClosedError(err) { log.Printf("Aborting stream for %s due to error: %v", string(metadata.Hash), err) }
            return
        }
...
func (s *Server) streamErasureCodedContent(w http.ResponseWriter, r *http.Request, metadata *pb.ContentMetadata) {
    ...
    for i, shardInfo := range metadata.Chunks {
        ...
        if err != nil { s.writeError(w, http.StatusInternalServerError, "Invalid shard hash", err); return }
        shardHashes[i] = h
    }
    shardsToFetch := int(metadata.DataShards)
    bitswapNode, ok := s.node.(interface{ GetBitswap() *bitswap.Engine })
    if !ok { s.writeError(w, http.StatusInternalServerError, "Node does not support bitswap", nil); return }
    blockChannel, err := bitswapNode.GetBitswap().GetBlocks(r.Context(), shardHashes)
    if err != nil { s.writeError(w, http.StatusInternalServerError, "Failed to start bitswap session for shards", err); return }
    availableShards := make([][]byte, len(shardHashes))
    shardMap := make(map[hasher.Hash]int)
    for i, h := range shardHashes { shardMap[h] = i }
    receivedCount := 0
    for blk := range blockChannel {
        if idx, ok := shardMap[blk.Hash()]; ok { availableShards[idx] = blk.RawData(); receivedCount++; if receivedCount >= shardsToFetch { go func(){ for range blockChannel{} }(); break } }
    }
    if receivedCount < shardsToFetch { s.writeError(w, http.StatusNotFound, "Could not retrieve enough shards to reconstruct file", nil); return }

    rangeHeader := r.Header.Get("Range")
    if rangeHeader != "" {
        start, end, err := parseRangeHeader(rangeHeader, metadata.Size)
        if err != nil { s.writeError(w, http.StatusBadRequest, "Invalid range header", err); return }
        reconstructedData, err := s.sharder.Reconstruct(availableShards)
        if err != nil { s.writeError(w, http.StatusInternalServerError, "Failed to reconstruct file from shards", err); return }
        if int64(len(reconstructedData)) > metadata.Size { reconstructedData = reconstructedData[:metadata.Size] }
        ...
        w.Header().Set("Content-Length", strconv.FormatInt(metadata.Size, 10))
        w.WriteHeader(http.StatusOK)
        if err := s.sharder.ReconstructToWriter(availableShards, w, int(metadata.Size)); err != nil { log.Printf("Failed to stream erasure reconstruction: %v", err) }
    }
}
...
func (s *Server) getChunksForRange(chunks []*pb.ChunkInfo, start, end int64) []*pb.ChunkInfo {
    ...
    for _, chunk := range chunks {
        ...
        if offset <= end && chunkEnd >= start { result = append(result, chunk) }
...
func (s *Server) prefetchChunks(ctx context.Context, chunks []*pb.ChunkInfo) error {
    ...
    for _, chunkInfo := range chunks {
        chunkHash, err := hasher.HashFromBytes(chunkInfo.Hash)
        if err != nil { continue }
        if _, exists := s.chunkCache.Get(chunkHash.String()); exists { continue }
        if has, _ := s.storage.Has(chunkHash); has { continue }
        wg.Add(1)
        go func(hash hasher.Hash) {
            defer wg.Done()
            select { case <-ctx.Done(): return; case sem <- struct{}{}: defer func(){ <-sem }() }
            if err := s.fetchChunk(ctx, hash); err != nil {
                errMu.Lock(); if firstErr == nil { firstErr = err }; errMu.Unlock()
...
func (s *Server) fetchChunk(ctx context.Context, hash hasher.Hash) error {
    if bitswapNode, ok := s.node.(interface{ GetBitswap() *bitswap.Engine }); ok {
        ...
        defer cancel()
        if _, err := bitswapNode.GetBitswap().GetBlock(fetchCtx, hash); err != nil { return fmt.Errorf("failed to fetch chunk %s: %w", hash, err) }
        return nil
    }
...
func (s *Server) streamChunkWithBuffer(ctx context.Context, w io.Writer, hash hasher.Hash, offset, length int, buffer []byte) error {
    ...
    if has, _ := s.storage.Has(hash); !has {
        if err := s.fetchChunk(ctx, hash); err != nil { return err }
    }
    blk, err := s.storage.Get(hash)
    if err != nil { return fmt.Errorf("failed to get block %s from storage: %w", hash, err) }
    data := blk.RawData()
    s.chunkCache.Put(hash.String(), data)
    if _, err := w.Write(data[offset : offset+length]); err != nil { return err }
    return nil
}
